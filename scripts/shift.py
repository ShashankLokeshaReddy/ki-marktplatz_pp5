...
class ShiftModel:
    ...
    def __init__(
        self, name, shift_name, current_shift_time=datetime.datetime(2022, 1, 1)
    ):
        try:
    ...
    def read_shifts_csv(self, name: str) -> pd.DataFrame:
            ...
            if isinstance(interval, str):
                return [
                    datetime.datetime.strptime(
                        str(interval).split("-")[0], "%H:%M"
                    ).time(),
                    datetime.datetime.strptime(
                        str(interval).split("-")[1], "%H:%M"
                    ).time(),
                ]
            else:
                return None
        ...
        if os.path.exists(csv_path):
            ...
            for i in range(6):
                shift_df.loc[:, "interval" + str(i)] = shift_df[
                    "interval" + str(i)
                ].apply(str_interval_to_datetime_interval)
            return shift_df
    ...
    def read_holidays_csv(self, name: str) -> list:
        ...
        csv_path = os.path.join(default_shifts_path, "holidays_" + name + ".csv")
        if os.path.exists(csv_path):
    ...
    def compute_work_period(self, start_time, work_time: float):
        ...
        while work_time:
            day_shift = self.shifts.loc[
                (self.shifts["shift"] == self.shift_name)
                & (self.shifts["weekday"] == current_datetime.weekday()),
                "interval0":"interval5",
            ].values[0]
    ...
    def count_time(self, start: datetime.datetime, end: datetime.datetime) -> int:
        ...
        while current_time < end:
            ...
            day_shift = self.shifts.loc[
                (self.shifts["shift"] == self.shift_name)
                & (self.shifts["weekday"] == current_time.weekday()),
                "interval0":"interval5",
            ].values[0]
            for interval in day_shift[day_shift != None]:
                shift_end = datetime.datetime.combine(current_time.date(), interval[1])
                ...
                if end >= shift_end:
                    ...
                    accumulated_seconds += (shift_end - current_time).total_seconds()
    ...
    def add_time(self, working_time: int) -> datetime.datetime:
        ...
        self.current_shift_time = self.get_earliest_time(self.current_shift_time)

        while working_time:
            day_shift = self.shifts.loc[
                (self.shifts["shift"] == self.shift_name)
                & (self.shifts["weekday"] == self.current_shift_time.weekday()),
                "interval0":"interval5",
            ].values[0]
            for interval in day_shift[day_shift != None]:
                shift_end = datetime.datetime.combine(
                    self.current_shift_time.date(), interval[1]
                )
                ...
                interval_minutes = (
                    shift_end - self.current_shift_time
                ).total_seconds() / 60
                if working_time >= interval_minutes:
                    ...
                    # Work stops during current interval
                    self.current_shift_time += datetime.timedelta(minutes=working_time)
                    working_time = 0
                    break
                # Check if current time is actually during a shift timeframe
    ...
    def get_earliest_time(self, start: datetime.datetime) -> datetime.datetime:
        ...
        temp_interval = self.shifts.loc[
            (self.shifts["shift"] == self.shift_name)
            & (self.shifts["weekday"] == (weekday + 1) % 6),
            "interval0",
        ].values[0]
        start = datetime.datetime(
            start.year,
...
if __name__ == "__main__":
    ...
    time_in_shift = shift.count_time(
        datetime.datetime(2022, 3, 9), datetime.datetime(2022, 3, 21)
    )
    print(
