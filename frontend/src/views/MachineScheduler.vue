...
export default defineComponent({
        ...
        return {
            ...
            eventResize: (info) => {
                ...
                if (allowedMachines.includes(selectedMachine)) {
                    ...
                    axios.post('http://' + window.location.hostname + ':8000/api/jobs/setInd/', formData)
                    .then(response => {
                        console.log(response.data);
                    })
                    .catch(error => {
    ...
    async created(){
        var response = await fetch('http://' + window.location.hostname + ':8000/api/jobs/getSchedule')
