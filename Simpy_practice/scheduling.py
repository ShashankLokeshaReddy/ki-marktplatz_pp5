...
def schedule_jobs(env, job_list, m_id):
  ...
  for job in job_list:
    ...
    release_date = pendulum.parse(job["jobInputDate"])
    deadline = pendulum.parse(job["deadlineDate"])
    duration = job["productionDuration"] * 60 # in minutes
    release_date_seconds = release_date.int_timestamp
    deadline_seconds = deadline.int_timestamp
    if (machine_id == m_id):
      # wait until release date to start the job
      yield env.timeout(max(0, release_date_seconds - env.now-pendulum.datetime(1970,1,1).int_timestamp))

      # start the job
      start_time = pendulum.from_timestamp(env.now).to_datetime_string()
      job["productionStart"] = start_time
      ...
      if end_time_comp > deadline_comp:
        ...
        deadlin_exceeded.append(job_id)
