...
class Job:
    ...
    def assign_resources(self, machines):
        for machine_id, machine in machines["machines"].items():
            if machine.count < machine.capacity:
                self.resource_id = machine_id
                with machine.request() as req:
                    yield req
                    self.production_start = self.env.now
                    if random.random() < 0.5:
                        for operator_id, operator in machines["operators"].items():
                            if operator.count < operator.capacity:
...
def run_simulation(env, jobs, machines):
    for job in jobs:
        env.process(job.assign_resources(machines[job.group]))
...
# Dictionary of machines, organized by group
machines = {
    "group1": machines_group1,
    "group2": machines_group2,
    "group3": machines_group3
}

# Run the simulation
run_simulation(env, jobs, machines)
