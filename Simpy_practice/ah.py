...
class Machine:
    ...
    def run_job(self, job):
        with self.machine.request() as req:
            ...
            current_time = datetime.fromtimestamp(self.env.now) #.strftime('%Y-%m-%d %H:%M:%S')
...
class ManufacturingLayer:
    ...
    def schedule_job(self, job):
        ...
        for machine_group in self.machine_groups:
            print(machine_group.group_name, " group ? ", job["group"])
...
class ProductionPlant:
    def __init__(self, env):
        self.env = env
        self.manufacturing_layer = ManufacturingLayer(env)
    
    def add_machine_group(self, machine_group):
        self.manufacturing_layer.add_machine_group(machine_group)
    
    def schedule_job(self, job):
        self.manufacturing_layer.schedule_job(job)
...
def main():
    ...
    schedule_jobs(env, ml1, job_list)
