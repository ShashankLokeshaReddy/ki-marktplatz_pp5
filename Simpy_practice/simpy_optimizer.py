...
# Job processing function containing with
# Result: No job start delay, but job overlapping is there
...
#     # current time is 1970, change it to release time (2021)
#     diff_delay = (release_time - current_time).total_seconds()
#     yield env.timeout(diff_delay)
#     with check_machine_availability(job["resourceId"], env, machines), check_operator_availability(job["operatorID"], env, operators):
...
#         yield env.timeout(job["productionDuration"] * 60) # minutes to seconds
#         job["productionEnd"] = datetime.fromtimestamp(env.now).strftime("%Y-%m-%d %H:%M:%S")
#         # machines[job["resourceId"]] = "available"
#         # operators[job["operatorID"]] = "available"
...
# Job processing function does not conatin with, executes in release order. 
# Result: Job start delay for few jobs, no overlapping
def process_job(job, env, machines, operators):
    release_time = string_to_timestamp(job["jobInputDate"])
    current_time = datetime.fromtimestamp(env.now) #.strftime('%Y-%m-%d %H:%M:%S')
    diff_delay = (release_time - current_time).total_seconds()
    yield env.timeout(diff_delay)
    machine_request = machines[job["resourceId"]].request()
    operator_request = operators[job["operatorID"]].request()
    yield machine_request & operator_request
    job["productionStart"] = datetime.fromtimestamp(env.now).strftime("%Y-%m-%d %H:%M:%S")
    print(f"Job {job['jobID']} for Part {job['partID']} on Machine {job['resourceId']} starts at {job['productionStart']}")
    yield env.timeout(job["productionDuration"] * 60) # minutes to seconds
    machines[job["resourceId"]].release(machine_request)
    operators[job["operatorID"]].release(operator_request)
    job["productionEnd"] = datetime.fromtimestamp(env.now).strftime("%Y-%m-%d %H:%M:%S")
    print(f"Job {job['jobID']} for Part {job['partID']} on Machine {job['resourceId']} ends at {job['productionEnd']}")
