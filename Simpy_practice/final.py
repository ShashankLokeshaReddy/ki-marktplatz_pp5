...
class Job:
    def __init__(self, env, job_id, part_id, group, job_input_date, deadline_date, production_duration):
        self.env = env
        self.job_id = job_id
        self.part_id = part_id
        self.group = group
        self.job_input_date = job_input_date
    ...
    def assign_resources(self, machine, operator):
        # self.resource_id = machine.machine_id
        with machine.request() as req:
            yield req
            self.production_start = self.env.now
            if random.random() < 0.5:
                with operator.request() as req:
                    yield req
                self.repair_time = 120 * 60 # simulate repair time
                yield self.env.timeout(self.repair_time)
            yield self.env.timeout(self.production_duration)
            self.production_end = self.env.now
...
class ManufacturingLayer:
    ...
    def add_machine_group(self, machine_group):
        self.machine_groups[machine_group.group_id] = machine_group

    def run_simulation(self, jobs):
        for job in jobs:
            machine_group = self.machine_groups[job.group]
            yield self.env.process(machine_group.assign_resources(job))

class MachineGroup:
    def __init__(self, env, group_id, machines, operators):
        self.env = env
        self.group_id = group_id
        self.machines = simpy.Resource(self.env, capacity=len(machines))
        self.operators = simpy.Resource(self.env, capacity=len(operators))

    def assign_resources(self, job):
        with self.machines.request() as machine_req:
            yield machine_req
            if random.random() < 0.5:
                with self.operators.request() as operator_req:
                    yield operator_req
                    yield self.env.process(job.assign_resources(self.machines, self.operators))
            # else:
            #     yield self.env.process(job.assign_resources(self.machines, None))

# initialize the environment
env = simpy.Environment()

# create the manufacturing layer
manufacturing_layer = ManufacturingLayer(env, "m_layer_1")

# create the machine groups
group1 = MachineGroup(env, "group1", ["M1_1", "M1_2", "M1_3"], ["O1"])
group2 = MachineGroup(env, "group2", ["M2_1", "M2_2"], ["O2"])
group3 = MachineGroup(env, "group3", ["M3_1", "M3_2", "M3_3", "M3_4"], ["O3"])

# add the machine groups to the manufacturing layer
manufacturing_layer.add_machine_group(group1)
manufacturing_layer.add_machine_group(group2)
manufacturing_layer.add_machine_group(group3)

# create the jobs and add them to the job queue
jobs = []
job_list = [
 {
   "id": 0,
...
for i, job_data in enumerate(job_list):
    job = Job(env, job_data["jobID"], job_data["partID"], job_data["group"], job_data["jobInputDate"], job_data["deadlineDate"], job_data["productionDuration"])
    jobs.append(job)

# run the simulation
env.process(manufacturing_layer.run_simulation(jobs))
env.run()

# print the results
for job in jobs:
    print("Job:", job.job_id)
    print("Part ID:", job.part_id)
    print("Group:", job.group)
    print("Input Date:", job.job_input_date)
    print("Deadline Date:", job.deadline_date)
    print("Production Duration:", job.production_duration)
    print("Resource ID:", job.resource_id)
    print("Production Start:", job.production_start)
    print("Production End:", job.production_end)
    print("Repair Time:", job.repair_time)
    print("\n")
