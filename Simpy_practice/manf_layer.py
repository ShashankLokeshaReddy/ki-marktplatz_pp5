...
def process_job(job, env, machines, operators):
    ...
    for key in machines:
        mach = machines[key]
        if mach.count < mach.capacity:
            job["resourceId"] = key
            with check_machine_availability(mach, env, machines) as machine:
                yield machine
                ...
                if random.random() < 0.5: # 10% chance of machine breaking down
                    ...
                    print("shashank operators", operators)
                    for key in operators:
                        op = operators[key]
                        if op.count < op.capacity:
                            with check_operator_availability(op, env, operators) as operator:
                                yield operator
                                yield env.timeout(repair_time)
                                print(f"Machine {job['resourceId']} fixed by operator {op} at {env.now}")
                            break
            ...
            break
# def job_scheduling(env, job, machine_group):
#     with machine_group['operators'][random.choice(list(machine_group['operators'].keys()))].request() as req_op:
#         yield req_op
#         with machine_group['machines'][random.choice(list(machine_group['machines'].keys()))].request() as req_mach:
#             yield req_mach
#             start = env.now
#             yield env.timeout(job['productionDuration'])
#             job['productionStart'] = start
#             job['productionEnd'] = env.now

# def schedule(env, job_list, machine_group):
#     for job in job_list:
#         if job['group'] == machine_group['group']:
#             env.process(job_scheduling(env, job, machine_group))
...
machines_group1 = {
    "group" : "group1",
    "machines" : {
    "M1_1": simpy.Resource(env, capacity=1),
    "M1_2": simpy.Resource(env, capacity=1),
    "M1_3": simpy.Resource(env, capacity=1)
    },
    "operators" : {
...
machines_group2 = {
    "group" : "group2",
    "machines" : {
    "M2_1": simpy.Resource(env, capacity=1),
    "M2_2": simpy.Resource(env, capacity=1),
    "M2_3": simpy.Resource(env, capacity=1)
    },
    "operators" : {
...
machines_group3 = {
    "group" : "group3",
    "machines" : {
    "M3_1": simpy.Resource(env, capacity=1),
    "M3_2": simpy.Resource(env, capacity=1),
    "M3_3": simpy.Resource(env, capacity=1),
    "M3_4": simpy.Resource(env, capacity=1)
    },
    "operators" : {
    "O3": simpy.Resource(env, capacity=1)
    }
}

m_layer1 = [machines_group1, machines_group2, machines_group3]
...
# schedule(env, job_list, machines_group1)
# schedule(env, job_list, machines_group2)
# schedule(env, job_list, machines_group3)

# def schedule(env, job, machines_group):
#     machine = None
#     production_start = None
#     production_end = None
    
#     with machines_group['machines']['M' + str(job['group'][-1]) + '_' + str(1)].request() as req:
#         yield req
#         machine = 'M' + str(job['group'][-1]) + '.' + str(1)
#         with machines_group['operators']['O' + str(job['group'][-1])].request() as req:
#             yield req
#             production_start = env.now
#             yield env.timeout(job['productionDuration'])
#             production_end = env.now
    
#     job['productionStart'] = production_start
#     job['productionEnd'] = production_end
#     job['machine'] = machine

# for job in job_list:
#     for m_group in m_layer1:
#         if m_group["group"] == job["group"]:
#             env.process(schedule(env, job, m_group["machines"], m_group["operators"]))

# env.run()

# for job in job_list:
#     print('Job: ', job['jobID'], ' | Machine: ', job['machine'], ' | Production Start: ', job['productionStart'], ' | Production End: ', job['productionEnd'])
...
for job in job_list:
    for m_group in m_layer1:
        if m_group["group"] == job["group"]:
            env.process(process_job(job, env, m_group["machines"], m_group["operators"]))
