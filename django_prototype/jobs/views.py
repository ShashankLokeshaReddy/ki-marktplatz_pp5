...
def baseline(self, sorting_tech):
    ...
    for job_data in jobs_data:
        ...
        job_data['start'] = datetime.strptime(str(job_data['start']), '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%dT%H:%M:%S.%fZ')
        job_data['end'] = datetime.strptime(str(job_data['end']), '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%dT%H:%M:%S.%fZ')
